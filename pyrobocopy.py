...
class PyRobocopier:
    ...
    def do_work(self):
        ...
        if not os.path.isdir(self.__dir2):
            if self.__maketarget:
                print
                'Creating directory', self.__dir2
                try:
    ...
    def __copy(self, filename, dir1, dir2):
        ...
        if self.__copyfiles:
            ...
            print
            'Copying file', filename, dir1, dir2
            try:
        ...
        if self.__updatefiles:
            ...
            if self.__copydirection == 0 or self.__copydirection == 2:
                ...
                if self.__cmptimestamps(st1, st2):
                    print
                    'Updating file ', file2  # source to target
                    try:
                        if self.__forcecopy:
                            os.chmod(file2, 0666)
                ...
                if self.__cmptimestamps(st2, st1):
                    print
                    'Updating file ', file1  # target to source
                    try:
                        if self.__forcecopy:
                            os.chmod(file1, 0666)
    ...
    def __dirdiff(self):
        ...
        if self.__dcmp.common:
            print
            'Common to', self.__dir1, ' and ', self.__dir2
            print
            ...
            print
            'No common files or sub-directories!'
    ...
    def synchronize(self):
        ...
        print
        'Synchronizing directory', self.__dir2, 'with', self.__dir1, '\n'
    ...
    def update(self):
        ...
        print
        'Updating directory', self.__dir2, 'from', self.__dir1, '\n'
    ...
    def dirdiff(self):
        ...
        print
        'Difference of directory ', self.__dir2, 'from', self.__dir1, '\n'
        self.__dirdiff()
    ...
    def report(self):
        ...
        print
        self.__numdirs, 'directories parsed,', self.__numfiles, 'files copied.'
        if self.__numdelfiles:
            print
            self.__numdelfiles, 'files were purged.'
        if self.__numdeldirs:
            print
            self.__numdeldirs, 'directories were purged.'
        if self.__numnewdirs:
            print
            self.__numnewdirs, 'directories were created.'
        if self.__numupdates:
            print
            self.__numupdates, 'files were updated by timestamp.'
        ...
        if self.__numcopyfld:
            print
            self.__numcopyfld, 'files could not be copied.'
        if self.__numdirsfld:
            print
            self.__numdirsfld, 'directories could not be created.'
        if self.__numupdsfld:
            print
            self.__numupdsfld, 'files could not be updated.'
        if self.__numdeldfld:
            print
            self.__numdeldfld, 'directories could not be purged.'
        if self.__numdelffld:
            print
            self.__numdelffld, 'files could not be purged.'
